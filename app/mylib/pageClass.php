...
class Paging{
    ...
    public function setHtml($sum = 0){
        ...
        $html = '';
        $html .= '<ul class="'.$this -> ul_class.'"pagination justify-content-center>';
        
        $aryList = [];
        
        if($this -> page != 1 && $this -> prev_str){
            $aryList[] = '<li class="page-item">'.$this -> getQuery(($this -> page - 1), $this -> prev_str).'</li>';
        }
        
        foreach($aryResult as $key => $value){
            if(is_numeric($value)){
                $aryList[] = '<li class="page-item">'.$this -> getQuery($value, $value).'</li>';
            }else{
                $aryList[] = '<li class="page-item">'.$this -> getQuery(0, $value).'</li>';
            }
        }
        
        if($this -> page != $max_page && $this -> next_str){
            $aryList[] = '<li class="page-item">'.$this -> getQuery(($this -> page + 1), $this -> next_str).'</li>';
        }
        
        $this -> html = '<nav aria-label="Page navigation example"><ul class="'.$this -> ul_class.' pagination justify-content-center">'.implode('', $aryList).'</ul></nav>';
    }
    ...
    private function getQuery($page, $str){
        ...
        if(!$this -> atag_remove_flg){
            ...
            if($aryQuery){
                return '<a class="page-link" href="'.$url.'?'.http_build_query($aryQuery).'">'.$str.'</a>';
            }else{
                return '<a class="page-link" href="'.$url.'">'.$str.'</a>';
            }
        }else{
